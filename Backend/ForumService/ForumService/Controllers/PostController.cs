...
namespace ForumService.Controllers
{
    ...
    public class PostController : ControllerBase
    {
        ...
        [HttpPost]
        [Authorize]
        public Task Post([FromBody]CreatePost command, CancellationToken token)
        {
            return _mediator.Send(new CreatePostCommand
            {
        ...
        [Authorize]
        [HttpPut("{PostId}")]
        public async Task<ActionResult<Unit>> Put(EditPost request, CancellationToken token) {
            try
            {
                return await _mediator.Send(new EditPostCommand
                {
