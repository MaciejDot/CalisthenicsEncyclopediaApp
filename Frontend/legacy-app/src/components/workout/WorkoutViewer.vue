<template>
  <b-overlay :show="workoutPlanIsLoading">
...
<script>
import { BContainer, BCard, BCol, BRow, BModal, BButton, BOverlay } from "bootstrap-vue";
...
export default {
  name: "WorkoutViewer",
  components: { BContainer, BCard, BCol, BRow, BModal, BButton, BOverlay },
  computed: mapState({
  ...
  methods: {
    ...
    edit() {
      this.$router.push({
        path: `/WorkoutCreator/${this.username}/${this.externalId}`
      });
    },
    showModal() {
    ...
    deletePlan() {
      this.$store
        .dispatch("deleteWorkoutPlanView", {
  ...
  mounted: function() {
    this.$store
      ...
      .then(data => {
        this.workoutPlanIsLoading = false;
